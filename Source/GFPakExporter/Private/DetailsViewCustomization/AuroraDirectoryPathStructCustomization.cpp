...
#include "AuroraDirectoryPathStructCustomization.h"
...
#define LOCTEXT_NAMESPACE "FAuroraDirectoryPathStructCustomization"

TSharedRef<IPropertyTypeCustomization> FAuroraDirectoryPathStructCustomization::MakeInstance()
{
	return MakeShareable(new FAuroraDirectoryPathStructCustomization());
}

void FAuroraDirectoryPathStructCustomization::CustomizeHeader( TSharedRef<IPropertyHandle> StructPropertyHandle, class FDetailWidgetRow& HeaderRow, IPropertyTypeCustomizationUtils& StructCustomizationUtils )
{
	...
	if(PathProperty.IsValid())
	{
		...
		if(bContentDir)
		{
			...
			.OnClicked( this, &FAuroraDirectoryPathStructCustomization::OnPickContent, PathProperty.ToSharedRef() )
			.ContentPadding(2.0f)
			.ForegroundColor( FSlateColor::UseForeground() )
			.IsFocusable(false)
			.IsEnabled(this, &FAuroraDirectoryPathStructCustomization::IsBrowseEnabled, StructPropertyHandle)
			[
				SNew(SImage)
		...
		else
		{
			...
			.OnClicked( this, &FAuroraDirectoryPathStructCustomization::OnPickDirectory, PathProperty.ToSharedRef(), bRelativeToGameContentDir, bUseRelativePath )
			.ContentPadding( 2.0f )
			.ForegroundColor( FSlateColor::UseForeground() )
			.IsFocusable( false )
			.IsEnabled( this, &FAuroraDirectoryPathStructCustomization::IsBrowseEnabled, StructPropertyHandle )
...
void FAuroraDirectoryPathStructCustomization::CustomizeChildren( TSharedRef<IPropertyHandle> StructPropertyHandle, class IDetailChildrenBuilder& StructBuilder, IPropertyTypeCustomizationUtils& StructCustomizationUtils )
{
}

FReply FAuroraDirectoryPathStructCustomization::OnPickContent(TSharedRef<IPropertyHandle> PropertyHandle) 
{
	TSharedPtr<IPropertyHandle> DirectoryPathHandle = PropertyHandle->GetParentHandle();
	check(DirectoryPathHandle);
...
FReply FAuroraDirectoryPathStructCustomization::OnPickDirectory(TSharedRef<IPropertyHandle> PropertyHandle, const bool bRelativeToGameContentDir, const bool bUseRelativePath) const
{
	FString Directory;
...
bool FAuroraDirectoryPathStructCustomization::IsValidPath(const FString& AbsolutePath, const bool bRelativeToGameContentDir, FText* const OutReason) const
{
	if(bRelativeToGameContentDir)
	{
...
void FAuroraDirectoryPathStructCustomization::OnPathPicked(const FString& Path, TSharedRef<IPropertyHandle> PropertyHandle)
{
...
bool FAuroraDirectoryPathStructCustomization::IsBrowseEnabled(TSharedRef<IPropertyHandle> PropertyHandle) const
{
